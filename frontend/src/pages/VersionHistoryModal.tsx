...
export default function VersionHistoryModal({ fileId, fileName, open, onClose }: Props) {
  ...
  return <Modal open={open} title={`Version History: ${fileName}`} onClose={onClose}>{isLoading ? <p>Loading…</p> : <div className="space-y-3">{data.map(v => <div key={v.id} className="flex items-center justify-between border-b pb-2"><div><b>v{v.version_number}</b><p className="text-sm text-gray-500">{v.upload_by_user?.full_name || '-'} · {new Date(v.upload_date).toLocaleString()} · {(Number(v.file_size) / 1024).toFixed(1)} KB</p><p className="text-sm">{v.remark || '-'}</p></div><button className="text-blue-600" onClick={() => download(v.id)}>Download</button></div>)}</div>}</Modal>;
}
